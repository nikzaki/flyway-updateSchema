create or replace view gsv_competition_status as select `c`.`id`                                                                    AS `id`,
                                             ...
                                             (select count(0)
                                              ...
                                              where ((`gs_competition_player`.`fk_competition` =
                                                      `c`.`id`) and
                                                     (`gs_competition_player`.`status` in ('R',
                                             ...
                                             (select count(0)
                                              ...
                                              where ((`wl`.`fk_competition` = `c`.`id`) and (`wl`.`status` = 'W')))      AS `total_waitlisted`
                                      ...
                                      union all select `c`.`id`                                                               AS `id`,
                                                       ...
                                                       (select count(0)
                                                        ...
                                                        where ((`gs_competition_player`.`fk_competition` =
                                                                `c`.`id`) and
                                                               (`gs_competition_player`.`status` in (
                                                                   'R',
                                                                   'X'))))                                                    AS `total_registered`,
                                                       `c`.`status`                                                           AS `competition_status`,
                                                       `c`.`competition_type`                                                 AS `competition_type`,
